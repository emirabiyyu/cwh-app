...
export default function ListLevelPage() {
  ...
  useEffect(() => {
    ...
    // Hitung total kata terkumpul saat ini
    ...
    allObjectsMap.forEach(obj => {
      if (localStorage.getItem(`collected_m${missionId}_${obj.id}`) === 'true') {
        currentCollectedCount++;
      }
    });

    const lastSeenCount = parseInt(localStorage.getItem(`collection_last_count_m${missionId}`) || '0', 10);
    if (currentCollectedCount > lastSeenCount) {
      setHasNewWords(true);
    }
  }, [mission, missionId]);
        ...
        <div className="w-full flex items-center mb-8">
          ...
          <div className="w-14 flex justify-end items-center">
            <button
              ...
              className="w-14 h-14 rounded-[1.25rem] bg-white border border-brown/10 shadow-sm flex items-center justify-center hover:bg-cream transition-all active:scale-95 relative"
            >
